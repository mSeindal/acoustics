...
def number_of_samples(sampling_rate: float, duration: float) -> int:
    return int(sampling_rate * duration)


def sine_wave(sampling_rate: float, duration: float, frequency: float) -> np.ndarray:
...
def white_noise(sampling_rate: float, duration: float) -> np.ndarray:
    noise = np.random.randn(number_of_samples(sampling_rate, duration))
...
def pink_noise(sampling_rate: float, duration: float) -> np.ndarray:
    ...
    num_samples = number_of_samples(sampling_rate, duration)
    num_rows = 16
...
def rectangular_pulse(
    ...
    num_samples = number_of_samples(sampling_rate, duration)
    pulse_samples = number_of_samples(sampling_rate, pulse_width)
...
def gaussian_pulse(
    ...
    t = np.linspace(0, duration, number_of_samples(sampling_rate, duration))
    pulse = np.exp(-0.5 * ((t - center) / width) ** 2)
    return pulse


def delta_function(sampling_rate: float, duration: float) -> np.ndarray:
    signal = np.zeros(number_of_samples(sampling_rate, duration))
    signal[0] = 1.0
    return signal


def unit_step_function(sampling_rate: float, duration: float) -> np.ndarray:
    signal = np.ones(number_of_samples(sampling_rate, duration))
    return signal
